...
> **⚠️ 嚴重警告：** 所有檔案名稱都必須是**小寫**（例如：`char000104.png`，不能是 `Char000104.png`）。此外，`.atlas` 檔案內引用的圖片檔名也必須是**小寫**。如果有任何大寫字母，遊戲將會**閃退**。
...
*   **Q: 裝完模組後遊戲閃退或崩潰了怎麼辦？**
    *   **A:** 這通常是因為模組的 `.png` 紋理檔案數量與遊戲原始設定不符。最新版的 App 已經會自動處理這個問題。如果您仍然遇到閃退，請嘗試對該模組使用 **「Merge Spine」** 功能手動合併。如果問題持續，那可能是模組本身損壞或與遊戲版本不相容。
