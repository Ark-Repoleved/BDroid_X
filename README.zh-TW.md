...
當您安裝的模組其實圖檔數量與遊戲原始檔案不符時（這通常是導致遊戲閃退的原因），本 App 會在打包過程中**自動偵測**並**即時合併**這些圖檔，您不再需要手動執行任何操作。
...
*   **Q: 裝完模組後遊戲閃退或崩潰了怎麼辦？**
    *   **A:** 這通常是因為模組的 `.png` 紋理檔案數量與遊戲原始設定不符。最新版的 App 已經會**自動處理**這個問題。如果您仍然遇到閃退，那很可能是模組本身有其他問題（例如檔案損壞或與遊戲版本不相容）。
